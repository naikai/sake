...
There are three different ways you can upload files into the `sake` pacakge. It can either be: 
...
![preload](Figures/Ting/preload.png)

A successfully loaded data will look like this  
![loaded_file](Figures/Ting/loaded_file.png)
...
- **DESeq-like normalization**: normalize the gene count using a method introduced in the [DESeq](http://bioconductor.org/packages/release/bioc/html/DESeq.html) package.
...
![Normalization](Figures/Ting/Normalization.png)
...
- **Variance stablizing transformation (VST)**: transform the gene count using the VST method available in the DEseq package.

- **Log transformation**: transform the gene count by *log2(count+1)*

![Transformation](Figures/Ting/Transformation.png)
...
An example QC plot displaying read distribution (x-axis) and gene coverage (y-axis) will be used for identifying potentially problematic samples. 

![QC_plot](Figures/Ting/QC_plot.png)

Samples with relatively low total transcript counts and gene coverage rates usually represent degraded or poorly amplified libraries. These can be identified visually and removed from the sample set before proceeding with downstream analyses. A selection box can be used to highlight samples in the left bottom corner. Samples within the selection box will be shown on the top right table. Users can then click on each library identified as problematic in the table at right. Clicking the *"Submit"*  button will discard all highlighted samples from further analysis in the SAKE pipeline. 
...
- **Upload gene list:** Users also have the option to upload a text file which contains the pre-determined genes of interest. This is typically done when you have a list of marker genes used to test a specific hypothesis and want to see how samples cluster accordingly. 
...
![Sample Correlation](Figures/Ting/Sample_Correlation.png)

In this case, we might estimate there are likely 5 major subclusters in this data set^[Ting *et al*, Single-Cell RNA Sequencing Identifies Extracellular Matrix Gene Expression by Pancreatic Circulating Tumor Cells, Cell Reports, 2014]. This is just to provide an overview of how the samples correlate with each other using `Pearson correlation`. However, `Pearson correlation` can easily be altered by a few genes with high expression. Therefore, we tend to be more confident about the clustering results identified from using the NMF method which will be described in the next section. (Figure 5)
...
In this example, we used the **Top MAD 1500** genes from the same dataset shown in the heatmap above for running this gene expression network module (Figure 6). 

![Gene network](Figures/Ting/Gene_network.png)

## Sample scatter plot

Users can also compare the expression levels of whole transcriptomes between two samples with a scatter plot. Genes that lie above the reference line are more highly expressed in the sample on the Y-axis, while genes that lie below the reference line are more highly expressed in the sample in the X-axis. Users can choose to log transform their gene counts, display *R^2^* coefficent of correlation, or show a two-fold change line to assess genes that have large changes in abundance between the two samples. 
...
![Consensus plot for estimating K](./Figures/Ting/NMF_Ting_EstimK_consensus.png)
![Summary stats for estimatin K](./Figures/Ting/NMF_Ting_EstimK_stats.png)
...
![NMF Groups](./Figures/Ting/NMF_Ting_K5_Group.png)
...
![Enriched features across NMF groups](./Figures/Ting/NMF_Ting_K5_Feature.png)
...
![t-SNE plots colored by NMF groups](Figures/Ting/NMF_Ting_K5_t-SNE.png)
...
![PCA plots colored by NMF groups](Figures/Ting/NMF_Ting_K5_PCA.png)
...
- **Filename:** User can modify the column names of the input gene count table to include experimental information that might be useful for comparisons or checkinh for batch effects. There can be mutliple categories separated by underscores, `_`. For example: `Type1` \_ `Batch1` \_ `Lane1`. 

![PCA plots colored by experimental information](Figures/Ting/NMF_Ting_K5_PCA_Filename.png)

- **Specific markers:** Users can also select specific markers from the drop down menu and check whether the expression of this gene/transcript is expressed in a particular NMF group. 

We will demonstrate by plotting *`Klf4`* (a key stem cell reprogramming factors marker) and *`Igfbp5`* (an extracellular growth factor binding protein) according to Ting et al, 2014. 

*`Klf4`* is enriched in `NMF group3`, where most of the samples in this group are marked as `CTC-c` in original published results. 

![PCA plots colored by expression level of *Klf4*](Figures/Ting/NMF_Ting_K5_PCA_Klf4.png)

*`Igfbp5`* is enriched in `NMF group3`, where most of the samples in this group are marked as `CTC-c` in original published results. 

![PCA plots colored by expression level of *Igfbp5*](Figures/Ting/NMF_Ting_K5_PCA_Igfbp5.png)
...
![Heatmap using NMF extracted features](Figures/Ting/NMF_Ting_K5_heatmap.png)
...
![Plots for summary stats](Figures/Ting/NMF_Ting_K5_groupstats.png)
...
![DESeq2 results for genes statistically enriched in NMF Group1](Figures/Ting/NMF_Ting_K5_DESeq2_Group1.png)
...
![Remeber to select correct species before running the analysis](Figures/Ting/NMF_Ting_K5_Enrich_Sel_Mouse.png)

![Enrichment results for KEGG pathways](Figures/Ting/NMF_Ting_K5_Enrich_Res.png)

User can also click on one of the enriched KEGG pathways they are interested in. Pathview^[https://bioconductor.org/packages/release/bioc/html/pathview.html] will maps and renders pathway graphs. In this case, we select `TGF-beta signaling pathway` from `NMF Group3`. Red color indicates genes that are upregulated in selected NMF cluster as compared to all the other clusters. 

![Pathway graph for *TGF-beta signaling pathway*](Figures/Ting/NMF_Ting_K5_Pathview.png)
